...
//	public void innitLevel1(String pathBox, String pathMonster, String pathItem, String level) {
//		try {
...
//		}
	...
	public void innitBomb() {
		...
		Bomb mBomb = new Bomb(x, y, mBomber.getSizeBomb(), 1600);
		arrBomb.add(mBomb);
	}
...
//	}
	...
	public boolean isImpactDoorVsBomer() {
		for (int i = 0; i < arrItem.size(); i++) {
			if (arrItem.get(i).isImpactItemVsBomber(mBomber)) {
				if (arrItem.get(i).getType() == Item.Door) {
					return true;
				}
			}
		}
	...
	public void checkWinAndLose() {
		...
		if (arrMonster.size() == 0 && nextRound == 0 && isImpactDoorVsBomer() == true ) {
			if (round == 3) {
				status = 3;
				nextRound++;
				GameSound.getIstance().getAudio(GameSound.PLAYGAME).stop();
